...
export default function CrearUsuarioModal({ isOpen, onClose, onUsuarioCreado }: CrearUsuarioModalProps) {
  ...
  useEffect(() => {
    const fetchSedesParaEmpresa = async () => {
      if (formData.empresa_id) {
        ...
        setSedes([]);
        setFormData(prev => ({ ...prev, sede_ids: [] }));
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nombre_completo || !formData.correo || !formData.password || formData.rol_ids.length === 0) {
                    ...
                    <div>
                        ...
                        <div className="relative mt-1.5">
                            ...
                            <input
                                ...
                                required
                                placeholder="••••••••"
                                className={`w-full pl-10 px-4 py-2.5 text-sm border rounded-lg transition-colors focus:ring-2 focus:ring-orange-200 focus:border-orange-500 ${passwordError ? 'border-red-400 bg-red-50' : 'border-slate-300'}`}
                            />
                        </div>
                        {passwordError && <p className="mt-1 text-xs text-red-500">{passwordError}</p>}
                        {!passwordError && <p className="mt-1 text-xs text-slate-400">Mínimo 6 caracteres</p>}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {formData.empresa_id !== null && (
                            <div className="md:col-span-2">
                                ...
                                {loadingSedes ? (
                                    <div className="flex items-center gap-2 h-10 text-slate-400 text-sm"><Loader2 className="w-4 h-4 animate-spin" /> Cargando sedes...</div>
                                ) : sedes.length === 0 ? (
                                    ...
                                    <div className="border border-slate-200 rounded-lg overflow-hidden">
                                        ...
                                        <div className="max-h-36 overflow-y-auto divide-y divide-slate-100">
                                            {sedesFiltradas.length === 0 ? (
                                                ...
                                                return (
                                                    <label key={s.id} className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors ${checked ? 'bg-orange-50' : 'hover:bg-slate-50'}`}>
