...
interface EditarEmpresaModalProps {
  isOpen: boolean;
  onClose: () => void;
  empresaData: EmpresaInput | null;
  onEmpresaEditada: () => void; 
}

import { STORAGE_URL } from '../../../../api/axiosInstance'

export default function EditarEmpresaModal({ isOpen, onClose, empresaData, onEmpresaEditada }: EditarEmpresaModalProps) {
  ...
  useEffect(() => {
    if (empresaData && isOpen) {
        setFormData({
            ...
            estado: empresaData.estado !== undefined ? empresaData.estado : 1,
            fecha_creacion: empresaData.fecha_creacion,
        });
        
        let initialLogo = null;
        if (empresaData.logo_path) {
            initialLogo = empresaData.logo_path.startsWith('/') 
              ? `${STORAGE_URL}${empresaData.logo_path}` 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await actualizarEmpresa(empresaData.id, submitData);
