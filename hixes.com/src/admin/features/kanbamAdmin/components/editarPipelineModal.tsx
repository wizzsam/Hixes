...
import { actualizarPipelineEtapa } from '../services/actualizarPipeline'
...
interface EditarPipelineModalProps {
    isOpen: boolean
    onClose: () => void
    etapaData: PipelineEtapa | null
    onEtapaEditada: () => void
}

export default function EditarPipelineModal({ isOpen, onClose, etapaData, onEtapaEditada }: EditarPipelineModalProps) {
    ...
    useEffect(() => {
        if (etapaData && isOpen) {
            setNombre(etapaData.nombre)
            setColor(etapaData.color ?? '#64748b')
        }
    }, [etapaData, isOpen])
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!etapaData?.id) return
        setIsLoading(true)

        try {
            const response = await actualizarPipelineEtapa(etapaData.id, { nombre: nombre.trim(), color })
            if (response.success) {
                ...
                if (response.errors) {
                    ...
                    toast.error(response.message || 'Error al actualizar la etapa.')
                }
            }
        } catch {
                                        ...
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <div>
                                                <Dialog.Title as="h3" className="text-xl font-bold text-slate-950">
                                                    Editar Etapa de Pipeline
                                                </Dialog.Title>
                                                <p className="mt-1 text-sm text-slate-600">
                                                    Modifica el nombre o color de la etapa.
                                                </p>
                                            </div>
                                        </div>
                                        <button
