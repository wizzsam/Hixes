...
import type { RecordatorioCashbackForm } from '../schemas/recordatorio.interface';
...
const DEFAULT_MENSAJE =
  'Hola {nombre}, recuerda que tienes un saldo de {monto} que vence el {vence} ({dias} días restantes). ¡Úsalo en nuestros beneficios!';

export default function ModalCrearRecordatorio({ isOpen, onClose, onCreado }: Props) {
  const [form, setForm] = useState<RecordatorioCashbackForm>({
    empresa_id: '',
    tipo_saldo: 'cashback',
    canal: 'whatsapp',
    mensaje_plantilla: DEFAULT_MENSAJE,
    dias_antes: 2,
    activo: true,
  });
  ...
  const handleClose = () => {
    setForm({ empresa_id: '', tipo_saldo: 'cashback', canal: 'whatsapp', mensaje_plantilla: DEFAULT_MENSAJE, dias_antes: 2, activo: true });
    onClose();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await recordatoriosService.crear(form);
      toast.success('Recordatorio creado exitosamente');
      onCreado();
      handleClose();
    } catch (err: any) {
      toast.error(err?.response?.data?.message ?? 'Error al crear el recordatorio');
    } finally {
      setLoading(false);
    }
  };
                ...
                <div className="px-6 py-5 border-b border-slate-200 bg-slate-50/50">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <Dialog.Title className="text-lg font-semibold text-slate-900">Nuevo Recordatorio</Dialog.Title>
                        <p className="text-sm text-slate-500">Configura cuándo y qué mensaje enviar</p>
                      </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1.5">
                      Enviar recordatorio con <span className="font-bold text-amber-600">{form.dias_antes}</span> día(s) de anticipación
                    </label>
                    <input
                      type="number"
                      min={1}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Hola {nombre}, tienes un cashback de {monto} por vencer..."
                    />
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-slate-700">{form.activo ? 'Activo — se enviará automáticamente' : 'Inactivo — no se enviará'}</span>
                  </div>
